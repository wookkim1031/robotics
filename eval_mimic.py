"""
Stage 3 — DeepMimic environment (track a reference motion under physics).

Stage 2 taught the G1 to stay upright. Stage 3 teaches it to IMITATE a clip:
the physical robot must reproduce the LAFAN1 walk you validated in Stage 1.

This fuses everything built so far:
    Stage 1 MotionLib (the reference)  +  Stage 0 Simulator (the physics)
    +  Stage 2 PPO (the learner)  ->  a physics-based imitation controller.
...
1. REFERENCE STATE INITIALIZATION (RSI). Reset to a RANDOM frame of the clip —
   ...
   every part of the motion (mid-stride, mid-turn) from the very first episode,
   states it could never reach on its own early in training.

2. TRACKING REWARD. A weighted sum of exp(-error) terms — joint pose, joint
   velocity, root position, root orientation. Each term is 1.0 at perfect match
   and decays as the robot drifts from the reference. Maximizing it = imitating.

3. FEEDFORWARD + RESIDUAL CONTROL. The PD target is the REFERENCE joint angles
   at the current motion time, plus a small learned offset. So action=0 already
   tries to hold the reference pose; the policy only learns the corrections that
   keep the whole body balanced and on-track. Huge head start vs. Stage 2.

4. EARLY TERMINATION on lost tracking. If the robot drifts too far from the
   reference (or falls), end the episode — focusing learning near the reference.

The observation includes (ref_pose - current_pose): the exact `tracking_diff`
signal from the ProtoMotions MimicADD code that started this whole project.
"""
...
class MimicEnv:
    def __init__(self, model_path, motion_lib, decimation=10, action_scale=0.25,
                 max_seconds=6.0, seed=0):
        self.sim = Simulator(model_path)
        self.motion = motion_lib
        self.rng = np.random.default_rng(seed)

        self.decimation = decimation
        self.action_scale = action_scale
        self.control_dt = self.sim.dt * decimation
        self.max_steps = int(round(max_seconds / self.control_dt))
    ...
    def step(self, action):
        ...
        # 'fell' is the standard true-terminal key PPO reads (here = lost tracking)
        info = {"fell": lost, "lost": lost, "timeout": self.step_count >= self.max_steps}
    ...
    def _get_obs(self):
        ...
        phase_obs = np.array([np.sin(2*np.pi*phase), np.cos(2*np.pi*phase)])  # (2)
    ...
    def _reward(self, action, lost):
        ...
        # DeepMimic-style: each term is 1.0 at perfect match, decays with error
...
if __name__ == "__main__":
    import sys
    from motion_lib import MotionLib, load_lafan1_g1_csv, make_synthetic_squat
    model = sys.argv[1] if len(sys.argv) > 1 else "_test_min.xml"
    if len(sys.argv) > 2:
        frames = load_lafan1_g1_csv(sys.argv[2]); fps = 30.0
    else:
        import mujoco as mj
        m = mj.MjModel.from_xml_path(model)
        frames = make_synthetic_squat(m.nq, m.nu, base_qpos=m.qpos0.copy()); fps = 30.0
    env = MimicEnv(model, MotionLib(frames, fps))
    print(f"obs_dim={env.obs_dim} action_dim={env.action_dim} "
          f"max_steps={env.max_steps} clip_dur={env.motion.duration:.2f}s")
    obs = env.reset()
    print("reset obs shape:", obs.shape)
    # zero action = pure feedforward (PD chases the reference pose, no learned help)
    for t in range(env.max_steps):
        obs, r, done, info = env.step(np.zeros(env.action_dim))
        if done:
            print(f"ended step {t}: lost={info['lost']} timeout={info['timeout']} r={r:.3f}")
            break